...
class ETLProcessor:
    ...
    def _group_listings_by_id(self, listings: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """
    ...
    def process_date(self, date: str, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if dry_run:
                logger.info(f"DRY RUN completed: would process {len(listings)} listings "
                           f"into {len(file_paths)} files")
            else:
                logger.info(f"ETL completed: processed {len(listings)} listings, "
    ...
    def process_date_range(
        ...
        summary = {
            ...
            "total_dates_processed": len([r for r in results.values() if "error" not in r]),
        ...
        logger.info(f"Date range processing completed: {summary['total_dates_processed']} "

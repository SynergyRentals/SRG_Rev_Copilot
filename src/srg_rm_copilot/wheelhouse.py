...
class WheelhouseClient:
    ...
    def _make_request(
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                raise WheelhouseRateLimitError(
    ...
    def get_all_listings_for_date(
        ...
        while True:
            try:
                data = self.get_listings(
                    date=date, limit=batch_size, offset=offset, filters=filters
                )
        ...
        logger.info(
            f"Successfully fetched {len(all_listings)} total listings for date {date}"
        )
        return all_listings

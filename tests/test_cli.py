...
class TestCLI:
    ...
    def test_etl_command_success(self, mock_etl_class, runner, mock_etl_success):
        ...
        mock_etl_instance.process_date.assert_called_once_with("2025-01-01", dry_run=False)
    ...
    def test_etl_command_dry_run(self, mock_etl_class, runner, mock_etl_success):
        ...
        mock_etl_instance.process_date.assert_called_once_with("2025-01-01", dry_run=True)
    
    @patch('src.srg_rm_copilot.cli.get_default_date')
    @patch('src.srg_rm_copilot.cli.ETLProcessor')
    def test_etl_command_default_date(self, mock_etl_class, mock_get_date, runner, mock_etl_success):
        ...
        mock_etl_instance.process_date.assert_called_once_with("2025-01-15", dry_run=False)
    ...
    def test_health_command_custom_output(self, mock_health_class, runner):
        ...
        mock_health_instance = Mock()
        mock_report = {
            "summary": {"total_files": 10, "total_size_mb": 50.0, "date_range": {"earliest": "2025-01-01", "latest": "2025-01-05"}},
        }
        mock_health_instance.generate_report.return_value = mock_report
        mock_health_class.return_value = mock_health_instance
        ...
        mock_health_instance.write_report.assert_called_with(mock_report, "/tmp/custom_health.json")
    
    @patch('src.srg_rm_copilot.cli.HealthMonitor')
    def test_health_command_error(self, mock_health_class, runner):
        """Test health command with error."""
        mock_health_instance = Mock()
        mock_health_instance.generate_report.side_effect = Exception("Health check failed")
        mock_health_class.return_value = mock_health_instance
    ...
    def test_etl_file_paths_output(self, mock_etl_class, runner):
        ...
        mock_result = {
            ...
            "file_paths": [f"data/raw/listing_{i}/2025-01-01.parquet" for i in range(15)],
            "base_path": "data",
        }
    ...
    def test_logging_setup_called(self, mock_setup_logging, runner):
        ...
        with patch('src.srg_rm_copilot.cli.HealthMonitor') as mock_health:
            mock_health_instance = Mock()
            mock_health_instance.generate_report.return_value = {"summary": {"total_files": 0, "total_size_mb": 0, "date_range": {"earliest": None, "latest": None}}}

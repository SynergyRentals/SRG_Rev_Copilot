...
class Config(BaseModel):
    ...
    openai_model: str = Field(
        default="gpt-4", description="OpenAI model to use for AI features"
    )

    # Data Storage Configuration
    data_base_path: str = Field(
        default="data", description="Base path for data storage"
    )

    # ETL Configuration
    etl_batch_size: int = Field(
        default=100, description="Number of listings to process per batch"
    )
    ...
    etl_timeout: float = Field(default=30.0, description="Request timeout in seconds")
    ...
    default_timezone: str = Field(
        default="America/Chicago", description="Default timezone for date operations"
    )

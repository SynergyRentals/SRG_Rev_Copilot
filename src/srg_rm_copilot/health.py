...
class HealthMonitor:
    ...
    def calculate_summary_stats(self, files_info: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        # Unique listings
        unique_listings = len(set(f["listing_id"] for f in files_info if f["listing_id"] != "unknown"))
        ...
        avg_rows_per_file = round(sum(valid_row_counts) / len(valid_row_counts), 1) if valid_row_counts else 0
    ...
    def check_data_freshness(self, files_info: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        # Get unique dates
        dates = set(f["date"] for f in files_info if f["date"] != "unknown")
    ...
    def analyze_listing_coverage(self, files_info: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for listing_id, files in listings_by_id.items():
            latest_file = max(files, key=lambda f: f["date"] if f["date"] != "unknown" else "0000-00-00")
        ...
        return {
            ...
            "most_active_listings": [{"listing_id": lid, "date_count": count} for lid, count in most_active],
            "recent_listings": [{"listing_id": lid, "latest_date": date} for lid, date in recent_listings],
        }
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        logger.info(f"Health report generated: {health_status} status with {len(issues)} issues")
        return report
